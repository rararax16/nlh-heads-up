...
ロビーの「対戦相手」で **AI と対戦（GTO 練習）** を選ぶと、AI（`GTO AI`）との
ヘッズアップが即座に始まります。AI の意思決定はサーバー（service_role）内でのみ行われるため、
カード秘匿のモデルは人間対戦と同一です（AI の手札はショーダウン公開までクライアントへ渡りません）。

**戦略の中身**（`server/game/ai/`）:

| 局面 | 手法 | 理論的位置づけ |
|------|------|--------------|
| プリフロップ ≦10〜12BB | fictitious play で解く **Nash プッシュ/フォールド均衡**（実効スタック・BBアンティを反映してランタイム計算、数十ms・キャッシュ付き） | ジャム/フォールドゲームの厳密な均衡 |
| プリフロップ ディープ | HU GTO の一般的頻度に基づくチャート（オープン約75%・3bet・「勝率×実現率 vs 価格」のディフェンス） | GTO 近似（混合戦略） |
| ポストフロップ | レンジ推定 + モンテカルロ勝率（高速7カード評価器） + ポットオッズ/頻度ベースの混合戦略 | GTO 近似 |

- 169×169 プリフロップ勝率表（`server/game/ai/data/preflopEquity.ts`）は各ペア4万サンプルの
  モンテカルロで事前生成（再生成: `GEN_EQUITY=1 pnpm vitest run scripts/gen-preflop-equity.test.ts`）
- 高速評価器は pokersolver とのランダム3000対戦一致テストで検証
- AI の手番は**サーバー駆動**: AI の手番が生まれるリクエスト（人間のアクション/配札/
  タイムアウト請求）の中でサーバーが同期的に AI を進めるため、常駐プロセスもクライアントの
  協力も不要。`POST /api/rooms/:code/ai-act`（冪等）はクライアントからの復旧ポーク用の保険で、
  さらにその先の安全網として既存タイムアウト処理が AI を自動チェック/フォールドさせる三重構え
...
- `tests/ai.test.ts` … AI の単体テスト（高速評価器 vs pokersolver 一致、勝率表の既知値、Nash 均衡の収束、AI 同士の全ハンド完走・合法性・チップ保存）
- `tests/integration.test.ts` … 作成→入室→開始→全ハンド消化→バスト決着、チップ総量保存、カード秘匿、AI 対戦フローを検証
...
│   │   └── ai/               #   GTO ベースの AI（Nash プッシュ/フォールド + MC 勝率）
│   │       ├── decide.ts     #     意思決定の入口（プリフロップ/ポストフロップ）
│   │       ├── nash.ts       #     ジャム/フォールド Nash 均衡ソルバー
│   │       ├── equity.ts     #     勝率表アクセス + ハンド序列
│   │       ├── mcEquity.ts   #     ポストフロップのモンテカルロ勝率
│   │       ├── fastEval.ts   #     高速7カード評価器
│   │       ├── handIndex.ts  #     169ハンドの正準インデックス
│   │       └── data/         #     事前生成した 169x169 プリフロップ勝率表
│   ├── utils/
...
タイムバンク、Realtime 同期、匿名認証、サーバー権威 + カード秘匿、AI 対戦（GTO ベース）。

**今後の候補**:
- AI の精度向上（ポストフロップをソルバー事前計算のルックアップ方式へ = フェーズ2）
